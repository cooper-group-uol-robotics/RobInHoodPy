...
# Core import
import pylabware.parsers as parser  # noqa: N813
from pylabware.controllers import (
    AbstractTemperatureController,
    in_simulation_device_returns,
)
from pylabware.exceptions import (
    SLConnectionError,
    SLDeviceCommandError,
    SLDeviceReplyError,
)
from pylabware.models import ConnectionParameters, LabDeviceCommands
...
class PetiteFleurChillerCommands(LabDeviceCommands):
    ...
    SET_RAMP_DURATION = {
        ...
        "type": str,
        "reply": {"type": str, "parser": parser.slicer, "args": [4, 8]},
    }  # from 0000 -> FFFF (65535) seconds
    START_RAMP = {
        "name": "{M5A",
        "type": str,
        "reply": {"type": str, "parser": parser.slicer, "args": [4, 8]},
    }  # associated with the target temperature in 16 bit hex
    GET_RAMP_TEMP = {
        "name": "{M5A****",
        "reply": {"type": str, "parser": parser.slicer, "args": [4, 8]},
    }
    ...
    KEY_LOCK = {
        ...
        "type": str,
        "reply": {"type": str},
    }  # Locks the manual interface in the system with 1
...
class PetiteFleurChiller(AbstractTemperatureController):
    ...
    def setup_device_with_default_settings(self):
    ...
    def is_idle(self) -> bool:
        ...
        if not self.is_connected():
            return False
        pressure = self.get_pump_pressure()
        return pressure < 5
    ...
    def start_temperature_regulation(self):
        ...
        # start circulation
        start_temp_ctrl = self.send(self.cmd.START_TEMP_CONTROL)
        ...
        start_circ = self.send(self.cmd.START_CIRCULATOR)
        sleep(5)
        return bool(int(start_circ and start_temp_ctrl))
    ...
    def stop_temperature_regulation(self):
        ...
        stop_circ = self.send(self.cmd.STOP_CIRCULATOR)
        # stop circulation
        stop_temp_ctrl = self.send(self.cmd.STOP_TEMP_CONTROL)
        return int(stop_circ and stop_temp_ctrl) == 0
    ...
    def set_temperature(self, temperature: float, sensor: int = 0):
        ...
        if -151 <= temperature <= 327:
            ...
            readback_temp = self.send(self.cmd.SET_TEMP_SP, f"{temperature:04X}")
            if readback_temp is None:
                raise SLDeviceReplyError(
                    f"Error setting temperature. Requested setpoint <{temperature}>, read back setpoint <{readback_temp}>"
                )
        else:
            raise SLDeviceCommandError("Temperature value OUT OF RANGE! \n")
    ...
    def ramp_temperature(self, end_temperature: float, time: int):
        ...
        if -32767 <= time <= 32767:
            ramp_duration_hex = (
                f"{time & 0xFFFF:04X}"  # convert to two's complement hex string
            )
            reply = self.send(self.cmd.SET_RAMP_DURATION, ramp_duration_hex)
            if (reply is not None) and (-151 <= end_temperature <= 327):
                end_temperature = int(
                    end_temperature * 100
                )  # convert to appropriate decimal format
                end_temperature_hex = f"{end_temperature & 0xFFFF:04X}"  # convert to two's complement hex string
                self.send(self.cmd.START_RAMP, end_temperature_hex)
            else:
                raise SLDeviceCommandError("The requested setpoint is out of range!")
        else:
            raise SLDeviceCommandError("The requested duration is out of range!")
    ...
    def get_status(self) -> str:
        ...
        return f"{int(status, 16) & 0b111111111111111:015b}"
    ...
    def set_circulator_control(self, pump_mode: int):
        ...
        self.send(self.cmd.SET_PUMP_MODE, f"{pump_mode & 0XFFFF:04X}")
