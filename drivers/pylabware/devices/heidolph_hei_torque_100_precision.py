...
from ..exceptions import SLConnectionError, SLDeviceInternalError, SLDeviceReplyError
...
class HeiTorque100PrecisionStirrerCommands(LabDeviceCommands):
    ...
    GET_STATUS = {
        "name": "f",
        "reply": {"type": str, "parser": parser.researcher, "args": [r"FLT:\s(.*!)"]},
    }
...
class HeiTorque100PrecisionStirrer(AbstractStirringController):
        ...
        if isinstance(reply, re.Match):  # type: ignore
            if reply is None:
                raise SLDeviceReplyError(
                    "Regular expression match failed on device reply!"
                )
            reply = reply[1]
    ...
    def check_errors(self):
        """Check device for errors & raises SLDeviceInternalError with
        ...
        if status == self.cmd.OVERHEAT_ERROR:
            ...
            raise SLDeviceInternalError(errmsg)
    ...
    def is_connected(self) -> bool:
        ...
        if self.cmd.DEFAULT_NAME in reply:
            return True
        self.logger.warning(
            "Device name <%s> doesn't seem to match device model!", reply
        )
        return False
    ...
    def stop_stirring(self):
        ...
        if readback_setpoint != 0:
            raise SLDeviceReplyError(
                f"Error stopping stirrer. Requested setpoint <{self._speed_setpoint}> RPM, "
    ...
    def set_speed(self, speed: int):
        ...
        if not self._running:
            ...
            if readback_setpoint != speed:
                self.stop()
                raise SLDeviceReplyError(
                    f"Error setting stirrer speed. Requested setpoint <{self._speed_setpoint}> "
