...
class RETControlViscHotplateCommands(LabDeviceCommands):
    ...
    GET_SPEED = {
        "name": "IN_PV_4",
        "reply": {"type": int, "parser": parser.slicer, "args": [-2]},
    }
...
class RETControlViscHotplate(AbstractHotplate):
    ...
    def is_connected(self) -> bool:
        ...
        # Check if the stirplate is likely to be an IKA RET Control Visc (based on firmware version) and rename it
            ...
            self.logger.warning(
                "is_connected()::An IKA RET hotplate with non-standard name has been detected."
                "Ensure that the right device is connected!"
                "The name will be now reset to default %s",
                self.cmd.DEFAULT_NAME,
            )
    ...
    @in_simulation_device_returns(value=25)
    def get_temperature(self, sensor: int = 1) -> float:
        ...
        if sensor == 0:
            ...
            raise SLDeviceCommandError(
                f"Invalid sensor provided! Allowed values are: {self.cmd.TEMP_SENSORS}"
            )
    ...
    @in_simulation_device_returns(value=500)
    ...
    @in_simulation_device_returns(value=2)
