...
class CVC3000VacuumPumpCommands(LabDeviceCommands):
    ...
    STATUSES = OrderedDict(
        [
            ...
            (
                ...
                {
                    0: "Control off.",
                    1: "Reaching set point/boiling point",
                    2: "Set point reached/boiling point found.",
    ...
    CONFIGURATIONS = OrderedDict(
        [
            ...
            (
                "inline_valve_connected",
                {0: "NO IN-LINE VALVE", 1: "IN-LINE VALVE CONNECTED"},
            ),
            (
                "coolant_valve_connected",
                {0: "NO COOLANT VALVE", 1: "COOLANT VALVE CONNECTED"},
            ),
            ("vent_valve_connected", {0: "NO VENT VALVE", 1: "VENT VALVE CONNECTED"}),
            (
                "fault_indicator_connected",
                {0: "NO FAULT INDICATOR", 1: "FAULT INDICATOR CONNECTED"},
            ),
            (
                "level_sensor_connected",
                {0: "NO LEVEL SENSOR", 1: "LEVEL SENSOR CONNECTED"},
            ),
            (
                "remote_module_connected",
                {0: "NO REMOTE MODULE", 1: "REMOTE MODULE CONNECTED"},
            ),
    ...
    GET_NAME = {
        "name": "IN_VER",
        "reply": {"type": str, "parser": parser.slicer, "args": [-7]},
    }
    GET_VERSION = {
        "name": "IN_VER",
        "reply": {"type": float, "parser": parser.slicer, "args": [11, 15]},
    }
    ...
    SET_MODE = {
        "name": "OUT_MODE",
        "type": int,
        "check": {"values": OPERATION_MODES},
        "reply": {"type": int},
    }
    SET_CONFIG = {
        "name": "OUT_CFG",
        "type": str,
    }  # TODO Probably develop a function for config (non needed atm)
    ...
    GET_PRESSURE = {
        "name": "IN_PV_1",
        "reply": {"type": float, "parser": parser.slicer, "args": [0, 6]},
    }
    GET_PRESSURE_SET = {
        "name": "IN_SP_1",
        "reply": {"type": int, "parser": parser.slicer, "args": [0, 4]},
    }
    ...
    SET_PRESSURE = {
        "name": "OUT_SP_1",
        "type": int,
        "check": {"min": 0, "max": 1060},
        "reply": {"type": int},
    }
    # Set the pressure with venting
    SET_PRESSURE_WITH_VENT = {
        "name": "OUT_SP_V",
        "type": int,
        "check": {"min": 0, "max": 1060},
        "reply": {"type": int},
    ...
    GET_PUMP_SPEED = {
        "name": "IN_PV_2",
        "reply": {"type": int, "parser": parser.slicer, "args": [3]},
    }
    GET_PUMP_SPEED_SET = {
        "name": "IN_SP_2",
        "reply": {"type": int, "parser": parser.slicer, "args": [3]},
    ...
    GET_OFF_PRESSURE = {
        "name": "IN_SP_5",
        "reply": {"type": int, "parser": parser.slicer, "args": [None, 4]},
    }
    SET_OFF_PRESSURE = {
        "name": "OUT_SP_5",
        "type": int,
...
class CVC3000VacuumPump(AbstractPressureController):
    ...
    def _recv(self, cmd: Dict) -> Union[int, float, str, bool]:
        ...
        except SLConnectionTimeoutError:
            raise SLDeviceReplyError(
                "No echo reply received from the device!"
            ) from None

    def _check_readback(self, readback: Any, value: Any) -> None:
        """Checks the CVC3000 echo reply against the parameter value sent.
        Raises SLDeviceReplyError in case readback value is wrong.
        """

        if self._simulation:
            self.logger.info(
                "SIM:: Assert readback <%s> equals value <%s>", readback, value
            )
        else:
            if value != readback:
                raise SLDeviceReplyError(
                    f"Read-back check failed! Expected <{value}>, read back <{readback}>."
                )
    ...
    def get_status(self, verbose: bool = False):
        ...
        if len(status) != len(self.cmd.STATUSES):
            raise SLDeviceReplyError(
                "Received status record length doesn't match expected!"
            )
    ...
    def get_configuration(self, verbose: bool = False):
        ...
        if len(cfg) != len(self.cmd.CONFIGURATIONS):
            raise SLDeviceReplyError(
                "Received configuration record length doesn't match expected!"
            )
        # Convert to list of integers
        # Hexadecimal only for language codes above 9 :/
        cfg = [int(x, base=16) for x in cfg]
        # Map to possible statuses dictionary
    ...
    def check_errors(self):
        ...
        if errors != 0:
            ...
            raise SLDeviceInternalError(errors_occurred)
    ...
    def start_pressure_regulation(self):
        ...
        if not self.is_idle():
            self.logger.warning(
                "Pump is already running, please, stop first before starting."
            )
        else:
    ...
    def set_mode(self, mode):
        ...
        if readback_mode != int(mode) and self.simulation is False:
            raise SLDeviceReplyError(f"Failed to switch pump to mode {mode}!")
    ...
    def get_pump_speed(self) -> int:
        ...
        if self._status["mode"] in (0, 3):
            raise SLDeviceCommandError(
                "Getting/setting pump speed is not supported in VacuuLAN/Auto modes!"
            )
    ...
    def set_end_pressure(self, pressure: int):
        ...
        if self._status["mode"] == 3 and pressure > 300:
            self.logger.error(
                "Due to FW error switch-off pressure in AUTO mode cannot be set above 300 mbar."
                "Falling back to 300."
            )
            pressure = 300
    ...
    def set_end_timeout(self, timeout: int):
        ...
        Exceptions:
            Raises SLDeviceCommandError if the timeout value is not within the range from 1 s
                to 86400 s.
        """
        ...
        if timeout < 1 or timeout > 86400:
            raise SLDeviceCommandError(
                f"Received invalid pump timeout value of {timeout} s. Pump timeout value must be withing 1 s to 86400 s (1 day)."
            )
