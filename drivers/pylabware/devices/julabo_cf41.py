...
class CF41ChillerCommands(LabDeviceCommands):
    ...
    STATUSES = {"00": "STOPPED", "02": "STOPPED", "01": "STARTED", "03": "STARTED"}
    ...
    GET_VERSION = {
        "name": "VERSION",
        "reply": {"type": str, "parser": parser.slicer, "args": [-3]},
    }
    # Get device name
    GET_NAME = {
        "name": "VERSION",
        "reply": {"type": str, "parser": parser.slicer, "args": [0, 23]},
    }
    ...
    SET_CONTROL_MODE = {
        "name": "OUT_MODE_08",
        "type": int,
        "check": {"values": CONTROL_DYNAMCIS_MODES},
    }
...
class CF41Chiller(AbstractTemperatureController):
    ...
    def check_errors(self):
        ...
        if status in self.cmd.ERRORS:
            ...
            raise SLDeviceInternalError(self.cmd.ERRORS[status])
        errmsg = f"Unknown status {status} received from device!"
        self.logger.error(errmsg)
        raise SLDeviceReplyError(errmsg)
    ...
    def set_regulation_mode(self, mode: int):
        ...
        if mode not in self.cmd.REGULATION_MODES:
            raise SLDeviceCommandError("Invalid regulation mode provided!")
    ...
    def set_temperature(self, temperature: float, sensor: int = 0):
        ...
        if setpoint_active == 0:
            ...
            raise SLDeviceCommandError(
                f"Invalid active SP <{setpoint_active}> received from the device!"
            )
    ...
    def get_temperature(self, sensor: int = 0) -> float:
        ...
        if mode is None:
            ...
            raise SLDeviceReplyError(
                f"Received invalid mode '{mode}' from the chiller "
        ...
        if sensor not in self.cmd.REGULATION_MODES.keys():
            raise SLDeviceCommandError(
                f"Invalid sensor number {sensor} provided!"
        ...
        if sensor != mode:
            self.logger.warning(
                "Chiller currently operates in {mode} regulation mode, "
    ...
    def ramp_temperature(self, end_temperature: float, time: float):
        ...
        if self.is_idle():
            self.logger.warning(
                "Chiller is not running! Please start the chiller before ramping the temperature."
            )
            return
        ...
        if ramp_step > 5.0 or ramp_step < -2.0:
            self.logger.error(
                "Ramp step <%s> °C/min is too steep to ramp reliably!", ramp_step
            )
        elif abs(ramp_step) < 0.05:
            self.logger.error("Ramp step <%s> °C/min is very low!", ramp_step)
        self.logger.debug(
            "ramp_temperature()::calculated ramp from <%s> to <%s> over <%s> minutes; step - <%s> degrees/min",
            start_temperature,
            end_temperature,
            time,
            ramp_step,
        ...
        ramp_thread = threading.Thread(
            target=self._ramp_runner,
            args=(start_temperature, ramp_step, end_temperature),
            daemon=True,
        )
        ramp_thread.start()
        return ramp_step
    ...
    def _ramp_runner(self, start: float, step: float, end: float):
        ...
        while (step > 0 and current_temperature <= end) or (
            step < 0 and current_temperature >= end
        ):
            self.logger.info(
                "Ramping from %s to %s, current step <%s>, %s minutes left",
                start,
                end,
                current_temperature,
                abs(round(end - current_temperature)),
            )
