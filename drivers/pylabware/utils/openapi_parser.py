...
class OpenAPIParser:
                ...
                for entry in endpoint_data:
                    for param_name, param_dict in entry["data"].items():
                        ...
                        if not isinstance(param_dict, dict):
                            self.logger.warning(
                                "run():: Wrong parameter configuration for %s %s.%s - not a dictionary!",
                                endpoint_name,
                                ".".join(entry["path"]),
                                param_name,
                            )
                        ...
                        self.all_commands.extend(
                            self.make_command(
                                endpoint_name, entry["path"], param_name, param_dict
                            )
                        )
            else:
                self.logger.warning(
                    "run():: No schema found for endpoint %s", endpoint_name
                )
    ...
    self.send(self.cmd.{2}, value)\n\n"""
        ...
        for cmd in self.all_commands:
            ...
            if cmd["method"] in API_GETTER_METHODS:
                stream.write(
                    getter.format(funcname, cmd["reply"]["type"].__name__, cmd["name"])
                )
            else:
                stream.write(setter.format(funcname, cmd["type"].__name__, cmd["name"]))

    def recursive_replace_refs(self, search_dict: dict | list, refs_dict: dict) -> None:
        ...
        for key, value in iter_seq:
            ...
            # If key is ref placeholder - replace
                ...
                replacement_node_path = value.strip(
                    CONFIG_REF_PATH_PREFIX + CONFIG_REF_PATH_SEPARATOR
                ).split(CONFIG_REF_PATH_SEPARATOR)
                self.logger.debug(
                    "recursive_replace_refs():: Replacement path: %s",
                ...
                self.logger.debug(
                    "recursive_replace_refs():: Search dict: %s", search_dict
                )
                self.logger.debug(
                    "recursive_replace_refs():: Replacement: %s", replacement_object
                )
                search_dict.pop(key)
    ...
    def recursive_find_dict_key(
        ...
        self.logger.debug(
            "recursive_find_dict_key():: Searching path %s\nTop-level keys: %s",
            path,
            list(keys),
        )
        for key in keys:
            ...
            if key == key_to_find:
                self.logger.debug(
                    "recursive_find_dict_key()::>>>>Found %s in %s", key_to_find, path
                )
                ...
                if isinstance(search_dict[key], dict):
                    ...
                    self.recursive_find_dict_key(
                        search_dict[key], key_to_find, path, results
                    )
                else:
    ...
    def recursive_reduce(self, search_dict, key_to_remove):
        ...
        for key, val in parent_dict.items():  # noqa: VNE002
            if key == key_to_remove:
                if isinstance(search_dict[key], list):
                    for item in search_dict[key]:
                        try:
                            ...
                            self.logger.error(
                                "recursive_reduce():: Can't update source dictionary with reduced value %s!",
                                item,
                            )
                else:
                    self.logger.error(
                        "recursive_reduce():: Can't reduce key %s with non-iterable value %s!",
                        key,
                        search_dict[key],
                    )
            elif isinstance(val, dict):
                search_dict[key] = self.recursive_reduce(val, key_to_remove)
        return search_dict
    ...
    def make_command(self, endpoint_name, schema_path, param_name, param_data):
        ...
        self.logger.info(
            "make_command():: Trying to build command for %s %s %s",
            endpoint_name,
            schema_path,
            param_name.upper(),
        )
        ...
        for param in subparams:
            ...
            if method in API_GETTER_METHODS:
                ...
                self.logger.warning(
                    "make_command():: Unknown method %s for endpoint %s, aborting command build.",
                    cmd["method"],
                    endpoint_name,
                )
                continue
            ...
            if cmd["method"] in API_SETTER_METHODS and command_data.get(
                "readOnly", False
            ):
                self.logger.info(
                    "Skipping read-only property %s in method %s",
                    cmd["path"],
                    cmd["method"],
                )
                continue
            ...
            commands.append(cmd)
            self.logger.info(
                "make_command():: Command %s built successfully.", cmd["name"]
            )
        return commands
...
if __name__ == "__main__":
    ...
    description = (
        "Translate OpenAPI-compatible specification into Pylabware-compatible command set.\n"
    ...
    epilog = (
        "Three files would be generated in the current directory:\n"
    ...
    parser.add_argument(
        "--yaml", metavar="yaml_file", type=str, help="Path to YAML file"
    )
    parser.add_argument(
        "--json", metavar="json_file", type=str, help="Path to JSON file"
    )
    ...
    fh = logging.FileHandler(
        f"openapi_parser_{time.strftime('%d_%m_%Y_%H-%M-%S', time.gmtime())}.log"
    )
    ...
    if arg.yaml:
        prog = OpenAPIParser(yamlfile=arg.yaml)
    elif arg.json:
        prog = OpenAPIParser(jsonfile=arg.json)
    else:
        parser.print_help()
        sys.exit(0)
    ...
    with open("commands.py", "w") as filestream:
        prog.print_commands(filestream)
        filestream.write("\n\n######################################\n\n\n")
        prog.print_getters_setters(filestream)

    # Dump full schema
    with open("full_schema.json", "w") as schema_file:
        schema_file.write(json.dumps(prog.openapi_paths, indent=2))
