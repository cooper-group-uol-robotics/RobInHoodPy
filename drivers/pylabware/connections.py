...
from .exceptions import (
    SLConnectionError,
    SLConnectionParametersErrors,
    SLConnectionProtocolError,
    SLConnectionTimeoutError,
    SLDeviceCommandError,
)
from .models import ConnectionParameters, LabDeviceReply
...
class AbstractConnection(ABC):
    ...
    def __init__(self, connection_parameters: ConnectionParameters):
        ...
        self.connection_parameters = {
            **self.DEFAULT_CONNECTION_PARAMETERS,
            **connection_parameters,
        }
        self.logger.info(
            "Creating connection object with the following settings: \n%s",
            self.connection_parameters,
        )
...
class SerialConnection(AbstractConnection):
    ...
    def open_connection(self):
        ...
        self._connection.timeout = self.connection_parameters.get(
        ...
        self._connection.inter_byte_timeout = self.connection_parameters.get(
            "inter_byte_timeout"
        )
        # Open connection
        try:
            ...
            raise SLConnectionError(
                f"Can't open serial port {self._connection.port}!"
            ) from e
        # Start connection listener
        self.listener = threading.Thread(
            target=self.connection_listener, name=f"{__name__}_listener", daemon=True
        )
        self._connection_close_requested.clear()
        self.listener.start()
        ...
        while True:
            ...
            if self._connection.in_waiting > 0:
                ...
                if self._data_ready.is_set() is True:
                    self.logger.warning(
                        "Discarding unconsumed device reply <%r>", self._last_reply
                    )
                self._last_reply = ""
                # Read data from connection into buffer
                while (
                    self._connection.in_waiting > 0
                    and len(self._last_reply) <= self.receive_buffer_size
                ):
                    self.logger.debug(
                        "connection_listener()::<%s> bytes to read",
                        self._connection.in_waiting,
                    )
                    # Lock connection
                    with self._connection_lock:
                        reply_bytes = self._connection.read(
                            size=self.receive_buffer_size
                        )
                        self.logger.debug(
                            "connection_listener()::got reply <%s>", reply_bytes
                        )
                    try:
                        self._last_reply += reply_bytes.decode(self.encoding, 'ignore')
                    except UnicodeDecodeError:
                        self.logger.exception(
                            "Can't decode device reply!", exc_info=True
                        )
    ...
    def close_connection(self):
        ...
        if self.is_connection_open():
            ...
            self.logger.warning(
                "Trying to close port <%s> that is not open", self._connection.port
            )
    ...
    def transmit(self, msg: str):
        ...
        if not self.is_connection_open():
            raise SLConnectionError("No connection to the device!")
        try:
            command = msg.encode(self.encoding)
        except SyntaxError:
            raise SLConnectionProtocolError(
                f"Can't encode command <{msg}> to a byte-string!"
            ) from None
        ...
        if delta < self.command_delay:
            self.logger.warning(
                "Command rate too high! Delaying next command for %s seconds",
                self.command_delay,
            )
            sleep(self.command_delay - delta)
        if self._data_ready.is_set() is True:
            self.logger.warning(
                "Previous reply <%s> has not been read.", self._last_reply
            )
        with self._connection_lock:
            self._connection.reset_input_buffer()
            self._connection.reset_output_buffer()
        ...
        if not self._data_ready.is_set():
            ...
            while self._data_ready.wait(timeout=self.receive_timeout * 10) is False:
                ...
                if failed_attempts > retries:
                    ...
                    raise SLConnectionTimeoutError("No reply received from the device!")
...
class TCPIPConnection(AbstractConnection):
    ...
    def open_connection(self):
        ...
        try:
            ...
            if self.protocol == "TCP":
                ...
                raise SLConnectionProtocolError(
                    f"Unknown transport layer protocol <{self.protocol}> provided!"
                )
            ...
            raise SLConnectionTimeoutError(
                f"Remote host {self.address}:{self.port} doesn't respond!"
            ) from e
        except (OSError, TypeError) as e:
            raise SLConnectionError(
                f"Can't open {self.protocol} socket for {self.address}:{self.port}!"
            ) from e
            ...
            with self._connection_lock:
                ...
                try:
                    ...
                    if chunk:
                        ...
                        if self._data_ready.is_set() is True:
                            self.logger.warning(
                                "Discarding unconsumed device reply <%r>",
                                self._last_reply,
                            )
                        self._last_reply = ""
                        try:
                            while chunk:
                                self.logger.debug(
                                    "connection_listener()::decoding chunk <%s>", chunk
                                )
                                self._last_reply += chunk.decode(
                        ...
                        # both on Windows and Linux
                            ...
                            self.logger.exception(
                                "Can't decode packet <%s>!", chunk, exc_info=True
                            )
                except (socket.timeout, BlockingIOError):
                    # No data in socket
                    pass
                except ConnectionAbortedError:
                    raise SLConnectionError("Device disconnected!") from None
    ...
    def close_connection(self):
        ...
        if self.is_connection_open():
            ...
            self.listener.join(
                timeout=max(self.receiving_interval, self.receive_timeout) * 5
            )
            self._connection.close()
    ...
    def transmit(self, msg: str):
        ...
        if self._data_ready.is_set() is True:
            self.logger.warning(
                "Previous reply <%r> has not been read.", self._last_reply
            )
        with self._connection_lock:
    ...
    def receive(self, retries: int = 3):
        ...
        if not self._data_ready.is_set():
            ...
            while self._data_ready.wait(timeout=self.receive_timeout * 50) is False:
                self.logger.debug("Failed attempt to recevive data...")
                failed_attempts += 1
                if failed_attempts > retries:
                    # No reply after timeout
                    raise SLConnectionTimeoutError("No reply received from the device!")
...
class HTTPConnection(AbstractConnection):
    ...
    def __init__(self, connection_parameters: ConnectionParameters):
        ...
        # Address settings
        self.address = (
            self.connection_parameters["address"]
            + ":"
            + str(connection_parameters["port"])
        )
        ...
        self.logger.debug(
            "HTTPConnection.__init__()::constructed base URL <%s>", self.base_url
        )
    ...
    def transmit(self, message: dict):  # type: ignore
        ...
        self.logger.debug(
            "transmit()::trying to invoke <%s> with method <%s>, data=<%s>",
            url,
            message["method"],
            message["data"],
        )
        try:
            ...
            raise SLConnectionTimeoutError("Can't connect to host!") from e
        # Log headers
        self.logger.debug("transmit()::reply headers::<%r>", reply.headers)
        # Check HTTP reply code
        if reply.status_code > 400:
            raise SLConnectionProtocolError(
                f"Server replied with HTTP code {reply.status_code} ({reply.text})"
            )
    ...
    def receive(self):
        ...
        try:
            ...
            raise SLConnectionProtocolError(
                f"Can't decode device reply!\n {self._last_reply}"
            )
        return LabDeviceReply(
            body=self._last_reply,
            parameters=self._last_reply_headers,
            content_type="json",
        )
...
@unique
class ModbusCommandType(Enum):
    WRITE_COILS = auto()
    WRITE_COIL = auto()
    READ_DISCRETE_INPUTS = auto()
    WRITE_REGISTER = auto()
    WRITE_REGISTERS = auto()
    READ_HOLDING_REGISTERS = auto()


class AbstractModbusConnection(AbstractConnection):
    """Base abstract class for all connection adapters."""

    class MsgType(BaseModel):
        address: int
        command_type: ModbusCommandType
        value: Optional[Union[int, bool, List[Union[int, bool]]]]
        unit: Optional[int]
        items_number: Optional[int]

    class ModbusSingleWrite(MsgType):
        value: Union[int, bool]

    class ModbusMultipleWrite(MsgType):
        value: List[Union[int, bool]]

    class ModbusRead(MsgType):
        """empy class just to have good naming and consisten code"""
    ...
    def __init__(self, connection_parameters: ConnectionParameters):
        ...
        self.modbus_client: BaseModbusClient = self.get_modbus_client(
            connection_parameters
        )

    def get_modbus_client(self, conn_params: ConnectionParameters) -> BaseModbusClient:
        "method incapsulate logic of creaction modbus client"
        # ! It is an interface. Must be implemented.
        return BaseModbusClient(None)

    def open_connection(self):
        """method runs the connection"""
        if not self.modbus_client.connect():
            raise SLConnectionError("Cannot connect to the microwave host")

    def close_connection(self):
        self.modbus_client.close()

    def is_connection_open(self):
        return self.modbus_client.is_connection_open()

    def get_msg_mapping(self) -> dict:
        msg_mapping = {
            ModbusCommandType.WRITE_COILS: {
                "msg": self.ModbusMultipleWrite,
                "method": self.modbus_client.write_coils,
            },
            ModbusCommandType.WRITE_COIL: {
                "msg": self.ModbusSingleWrite,
                "method": self.modbus_client.write_coil,
            },
            ModbusCommandType.READ_DISCRETE_INPUTS: {
                "msg": self.ModbusRead,
                "method": self.modbus_client.read_discrete_inputs,
            },
            ModbusCommandType.WRITE_REGISTER: {
                "msg": self.ModbusSingleWrite,
                "method": self.modbus_client.write_register,
            },
            ModbusCommandType.WRITE_REGISTERS: {
                "msg": self.ModbusMultipleWrite,
                "method": self.modbus_client.write_registers,
            },
            ModbusCommandType.READ_HOLDING_REGISTERS: {
                "msg": self.ModbusRead,
                "method": self.modbus_client.read_holding_registers,
            },
        }
        return msg_mapping

    def validate_command_type(self, command_type, raise_exception=True) -> bool:
        if not isinstance(command_type, ModbusCommandType):
            if raise_exception:
                raise TypeError(
                    "arg command_type should an instance of ModbusCommandType"
                )
    ...
    def extract_modbus_msg_type(self, command_type: ModbusCommandType) -> Type[MsgType]:
        self.validate_command_type(command_type)

        return self.get_msg_mapping()[command_type]["msg"]

    def extract_modbus_method(
        self, command_type: ModbusCommandType
    ) -> Callable[..., Any]:

        self.validate_command_type(command_type)

        return self.get_msg_mapping()[command_type]["method"]

    def get_modbus_msg(self, msg: dict) -> MsgType:
        """validates the given msg and return corresponding modbus msg class

        Args:
            msg (dict): event msg from a device

        Raises:
            SLDeviceCommandError: exception in case of wrong msg type

        Returns:
            MsgType: Parrent class of all modbus msgs.
        """
        try:
            class_msg = self.MsgType(**msg)
            modbus_msg_type = self.extract_modbus_msg_type(class_msg.command_type)

            modbus_msg = modbus_msg_type(**class_msg.dict())

            return modbus_msg

        except ValidationError as e:
            raise SLDeviceCommandError(e.json())

        except KeyError as e:
            raise SLDeviceCommandError(e)

    def get_data_from_modbus_msg(self, modbus_msg: MsgType) -> dict:
        """extracts and returns data for modbus client transfer method

        Args:
            modbus_msg (MsgType): Modbus msg type

        Returns:
            dict: data for modbus client (structure depends on modbus_msg)
        """
        msg = modbus_msg.dict()

        data = {}
        for key, value in msg.items():
            if key != "command_type" and value is not None:
                data[key] = value

        return data

    def transmit_msg(self, modbus_msg: MsgType):
        """sends data to the device

        method extracts modbus client based on command_type
        then extracts data from modbus_msg
        then sends it to a device,
        and finally if command_type is read it saves a response to the local property

        Args:
            modbus_msg (MsgType): modbus msg type defined in the current class

        Raises:
            SLDeviceInternalError: in case of error in a response msg it will raise this exception
        """

        modbus_command_method = self.extract_modbus_method(modbus_msg.command_type)

        data = self.get_data_from_modbus_msg(modbus_msg)
        response = modbus_command_method(**data)

        if "READ" in modbus_msg.command_type.name:
            self._register_value = response.registers

    def transmit(self, msg: dict):  # type: ignore
        """Transmits msg to the device."""

        modbus_msg = self.get_modbus_msg(msg)

        with self._connection_lock:
            self.transmit_msg(modbus_msg)

    def receive(self) -> LabDeviceReply:
        # ToDo_nikita: think about more advance data structure for body (just an idea)
        return LabDeviceReply(body=self._register_value, content_type="list")


class ModbusTCPConnection(AbstractModbusConnection):
    ...
    def get_modbus_client(self, conn_params: ConnectionParameters) -> BaseModbusClient:
        try:
            modbus_tcp_settings = ModbusTCPConnection.ModbusTCPSettings(
                port=conn_params["port"], host=conn_params["address"]
            )
        # ToDo_Nikita: this is the closest exception from the list, but I don't think it suits here
        except ValidationError as e:
            raise SLConnectionParametersErrors(f"Cannot create client class {e.json()}")

        except KeyError as e:
            raise SLConnectionParametersErrors("Cannot create client class {}", e)

        return ModbusTcpClient(
            port=modbus_tcp_settings.port, host=str(modbus_tcp_settings.host)
        )


class ModbusSerialConnection(AbstractModbusConnection):
    class ModbusSerialSettings(BaseModel):
        port: str
        baudrate: int
        parity: str
        method: str
    ...
    def get_modbus_client(self, conn_params: ConnectionParameters) -> BaseModbusClient:
        try:
            modbus_tcp_settings = ModbusSerialConnection.ModbusSerialSettings(
                port=conn_params["port"],
                baudrate=conn_params["baudrate"],
                method=conn_params["method"],
                parity=conn_params["parity"],
            )
        ...
        return ModbusSerialClient(**modbus_tcp_settings.dict())


class SOAPConnection(AbstractConnection):
    SOAP_DEFAULT_CONNECTION_PARAMETERS = {
        "wsdl_name": "",
        "password": None,
        "socket": "",
        "binding": "",
    ...
    def __init__(self, connection_parameters: ConnectionParameters):
        ...
        # Init base class, set common parameters
        config = {**self.SOAP_DEFAULT_CONNECTION_PARAMETERS, **connection_parameters}
        super().__init__(config)

        # Reply for the SOAP response
        self._last_reply = ""

        # Put connection parameters for the SOAP connection in
        self.wsdl_name = self.connection_parameters["wsdl_name"]
        self.password = self.connection_parameters["password"]
        self.socket = self.connection_parameters["socket"]
        self.binding = self.connection_parameters["binding"]
    ...
    def open_connection(self):
        return "Use create service instead"

    def create_service(self, service: str) -> Any:
        """
        Creates a service object, needed for when the wsdl doesn't define any
        service elements. Valid services are defined in the WSDL and in the
        specific device file.
            Args:
                service(str): service in the WSDL file
        ...
        service_binding = self.binding + service
        binding_name = namespace + service_binding

        service_object = self.client.create_service(binding_name, endpoint)
    ...
    def is_connection_open(self):
        """If possible for a given service implement in the device class"""

    def transmit(self, msg: dict):
        """Sends the message from the client to the server. The message is a combination of
        service and the method. A bound_method object is created with getattr, which then is called.
        If the method takes arguments these are passed into it as a list on calling the bound method.
        The response is generated alongside the transmisson and so is saved to self._last_reply for later reading"""

        service_object = msg["service_object"]
        method = msg["method"]

        if "message_list" in msg.keys():
            msg_list = msg["message_list"]
            bound_method = getattr(service_object, method)
            response = bound_method(*msg_list)
    ...
    def _dict_search(self, dictionary: dict, search_key: str) -> dict:
        """Searches a inhomogously nested dictionary for key value pairs.
        Returns all found key value pairs in a dictionary.
            Args:
                dictionary(dict): dictionary to do nested search
                search_key(str): key value to be searched
        """
        ...
        if hasattr(dictionary, 'items'):
            for key, value in dictionary.items():
                ...
                if isinstance(value, dict):
                    for result in self._dict_search(search_key, value):
                        yield result
                elif isinstance(value, list):
                    for element in value:
                        for result in self._dict_search(search_key, element):
                            yield result

        return result

    # TODO add some searching of the reply here
    def receive(self) -> LabDeviceReply:
        """ Takes the request from the self._last_reply object and writes
        it into a LabDeviceReply object.
        """
