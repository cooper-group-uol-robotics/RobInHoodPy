...
from .exceptions import SLDeviceCommandError, SLDeviceError, SLDeviceReplyError
    ...
    def wrapper(func):
        ...
        def wrapper_inner(*args, **kwargs):
            ...
            if simulation is True:
                ...
                try:
                    if "{$args[" in value:
                        ...
                        try:
                            ...
                            slf.logger.error(
                                "SIM:: Can't extract argument number from {$args[]}, check syntax!"
                            )
                            dec_retval = None
                        try:
                            ...
                            slf.logger.error(
                                "SIM:: Can't find argument number %s in arguments list <%s>!",
                                value,
                                args,
                            )
                            dec_retval = None
                ...
                slf.logger.info(
                    "SIM :: Patched send() to return <%s>, calling <%s>",
                    value,
                    func.__name__,
                )
...
class LabDevice(AbstractLabDevice):
    ...
    def __init__(
        ...
        self.logger = logging.getLogger(
            __name__ + "." + self.__class__.__name__ + "." + self.device_name
        )
        ...
        elif connection_mode == "modbustcp":
            self.connection = ModbusTCPConnection(connection_parameters)
        elif connection_mode == "SOAP":
            self.connection = SOAPConnection(connection_parameters)
        else:
            raise SLDeviceError(
                f"Unsupported connection mode <{connection_mode}> for <{self.device_name}>"
            )
    ...
    def connect(self):
        ...
        except (SLConnectionError, SLConnectionTimeoutError) as e:
            raise SLDeviceError(
                f"Can't connect to device <{self.__class__.__name__}.{self.device_name}>!"
            ) from e
    ...
    def disconnect(self):
        ...
        if self._background_tasks:
            self.logger.info(
                "Background tasks running, stopping them before disconnect."
            )
            self.stop_all_tasks()
    ...
    def check_value(self, cmd: Dict, value: Any) -> Any:
        ...
        if "type" in cmd.keys() and cmd["type"] is not None:
            try:
                ...
                self.logger.debug(
                    "check_value()::type casted value <%s> to <%s>.", value, cmd["type"]
                )
            # Invalid type definition
            except TypeError:
                self.logger.error(
                    "check_value()::Illegal type <%s> specification in command <%s> definition.",
                    cmd["type"],
                    cmd["name"],
                )
            # Type cast error
                ...
                raise SLDeviceCommandError(
                    f"Can't cast value <{value}> to type <{cmd['type']}>."
                )
        else:
        ...
        if "check" in cmd.keys() and cmd["check"] is not None:
            ...
            try:
                if value < cmd["check"]["min"]:
                    raise SLDeviceCommandError(
                        f"Requested value <{value}> is below limit <{cmd['check']['min']}> !"
                    )
                self.logger.debug(
                    "check_value()::min check <%s> < <%s>", value, cmd["check"]["min"]
                )
                if value > cmd["check"]["max"]:
                    raise SLDeviceCommandError(
                        f"Requested value <{value}> is above limit <{cmd['check']['max']}> !"
                    )
                self.logger.debug(
                    "check_value()::max check <%s> > <%s>", value, cmd["check"]["max"]
                )
            # No cmd["check"]["min"] or cmd["check"]["max"]
            except KeyError:
                self.logger.debug(
                    "check_value()::min/max check not required - skipped."
                )
            # Invalid value in cmd["check"]["min"] or cmd["check"]["max"]
            except TypeError:
                self.logger.error(
                    "Illegal min/max values specification in command <%s> definition!",
                    cmd["name"],
                )
            ...
            try:
                if value not in cmd["check"]["values"]:
                    raise SLDeviceCommandError(
                        f"Requested value <{value}> not in the allowed range <{cmd['check']['values']}>."
                    )
                self.logger.debug(
                    "check_value()::range check <%s> in range <%s>",
                    value,
                    cmd["check"]["values"],
                )
            # No cmd["check"]["range"]
                ...
                self.logger.error(
                    "Illegal range specification in command <%s> definition.",
                    cmd["name"],
                )

        # rjust the values presented here
        if "rjust" in cmd.keys() and cmd["check"] is not None:
            try:
                value = str(value).rjust(cmd['rjust'], "0")
                self.logger.debug(
                    f"{value} adjusted with {cmd['rjust']} leading zeros")
    ...
    # TODO expand returns to include dicts?
    ...
    def parse_reply(self, cmd: Dict, reply: Any) -> Any:
        ...
        Args:
            ...
            cmd: Command definition to look for the parsing workflow in.
        ...
        try:
            ...
            self.logger.debug(
                "parse_reply()::got parser <%s>, arguments <%s>", function, args
            )
            # Run parsing
            if callable(function):
                ...
                self.logger.error(
                    "Parsing function <%s> defined for command <%s> is not callable!",
                    function,
                    cmd["name"],
                )
        except KeyError:
            ...
            self.logger.debug(
                "parse_reply()::parsing not defined for command <%s> - skipped.",
                cmd["name"],
            )
    ...
    def cast_reply_type(self, cmd: Dict, reply: Any) -> Union[int, float, str, bool]:
        ...
        try:
            ...
            self.logger.debug(
                "cast_reply_type()::casted reply type to %s.", cmd["reply"]["type"]
            )
        ...
        # cmd["reply"]["type"] does not point to a proper data type
            ...
            self.logger.error(
                "Illegal parse type <%s> specification in command <%s> definition.",
                cmd["reply"]["type"],
                cmd["name"],
            )
            return reply
        # Type casting error
            ...
            raise SLDeviceReplyError(
                f"Can't cast reply <{reply}> to type <{cmd['reply']['type']}>."
            )
        else:
            return casted_reply
    ...
    def wait_until_ready(self, check_ready: Optional[Callable] = None):
        ...
        """
        return self.execute_when_ready(
            action=lambda *args: None, check_ready=check_ready
        )

    def execute_when_ready(
        self, action: Callable, *args, check_ready: Optional[Callable] = None
    ):
...
class LabDeviceTask(threading.Thread):
    ...
    def __init__(self, interval: int, method: Callable, args=None):
        ...
        # Logger object
        self.logger = logging.getLogger(
            __name__ + "." + self.__class__.__name__ + "." + self.method.__name__
        )
    ...
    def run(self):
        ...
        self.logger.info(
            "Background task %s started. Executing <%s> command every <%s> seconds.",
            threading.get_ident(),
...
class AbstractSolidDispensing(LabDevice):
    """Any device capable of dispensing solid."""
